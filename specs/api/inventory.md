...
| `GET` | `/api/settings` | `shared` | [2073](../../server/index.ts#L2073) | Returns active gameshow's global settings (resolved `showTitle` — landing-page heading / gamemaster label / tab title, active key, team sizes, point system, teamRandomizationEnabled — forced false below 2 teams, joker config incl. `jokersInLastGame` + operator-editable `jokerRules` explanation text, activeGameshowTitle, isCleanInstall, gameCount, `players` roster for the HomeScreen randomization prefill, `teamCount` 0-4, `pointMode` — how the gameshow scores (`positional` / `flat` / `per-correct-answer`), `incompatibleGames` — the gameOrder entries that cannot be scored at that count, and `teamColors` — the operator's per-team accent colours with the master switch already applied). | — | `SettingsResponse` |
