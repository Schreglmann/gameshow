...
| `GET` | `/api/settings` | `shared` | [2073](../../server/index.ts#L2073) | Returns active gameshow's global settings (resolved `showTitle` — landing-page heading / gamemaster label / tab title, active key, team sizes, point system, teamRandomizationEnabled — forced false below 2 teams, joker config incl. `jokersInLastGame` + operator-editable `jokerRules` explanation text, activeGameshowTitle, isCleanInstall, `totalGames` — the show's game count, sizing the header counter in the team-name length check, `players` roster for the HomeScreen randomization prefill, `teamCount` 0-4, `pointMode` — how the gameshow scores (`positional` / `flat` / `per-correct-answer`), `incompatibleGames` — the gameOrder entries that cannot be scored at that count, and `teamColors` — the operator's per-team accent colours with the master switch already applied). | — | `SettingsResponse` |
| `GET` | `/api/run-of-show` | `shared` | [server/index.ts](../../server/index.ts) | The active gameshow's running order with resolved titles — backs the gamemaster's "Ablauf" panel, which otherwise only ever mirrors the CURRENT game. One entry per `gameOrder` slot; an unresolvable ref is returned as a `missing` entry rather than dropped, so every `index` still matches its `gameOrder` position. See [specs/gamemaster-run-of-show.md](../gamemaster-run-of-show.md). | — | `RunOfShowResponse` |
...
| `gamemaster-command` | C→S→C | **no** (ephemeral) | gamemaster PWA | `shared` (gamemaster writes, show reads) | One-shot command from gamemaster to show (`next`, `award`, `use-joker`, ...), including the non-linear jumps `goto:home` / `goto:rules` / `goto:game-<index>` / `goto:summary` — see [specs/gamemaster-run-of-show.md](../gamemaster-run-of-show.md). |
