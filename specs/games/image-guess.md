...
- [ ] Honours the shared `randomizeQuestions` / `questionLimit` config (via `useQuestionOrder`): the example question stays first, disabled questions are filtered, the rest are shuffled with a session-stable order and optionally capped
- [ ] A question with no explicit `obfuscation` gets a random effect seeded off its slot, so a live edit never re-rolls the effect of the image on screen — see [live-question-order.md](../live-question-order.md)
