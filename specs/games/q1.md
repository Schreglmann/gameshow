...
## Naming note
Originally named `four-statements`. Renamed to `q1` when a new clue-based `four-statements` game type was introduced. Runtime behaviour is unchanged.
...
## State / data changes
- No `AppState` changes
- Config type: `Q1Config` in `src/types/config.ts` (`type: 'q1'`)
- Question type: `Q1Question`
...
## UI behaviour
- Component: `src/components/games/Q1.tsx`
