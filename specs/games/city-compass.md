...
- [x] The crop stays **horizontally centered on the center city**: each half-width of the box is the larger of the two sides, so the `?` sits at the middle of the SVG and therefore at the middle of the screen. The labels around the ring are never symmetric, and a box tight on both sides pushed the `?` off-center by however much the names on one side outran the other. The cost is a little air on the shorter side
- [x] Vertically the crop stays **tight**, with less padding than the sides: nothing is centered against the middle of the screen on that axis, so squaring up the taller side would only add empty space above or below the drawing — and height is what the rose runs out of first on the stage. The side padding stays wider because the label widths are an estimate, while the vertical extents are a cap height and a descender
