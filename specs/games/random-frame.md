...
- Auto-scroll mirrors the other quiz games via `useQuizAutoScroll(`${qIdx}:${showAnswer}`, showAnswer ? 'answer' : 'top', showAnswer ? 'smooth' : 'instant')`: during the question phase the (up-to-60vh) frame is anchored **instantly** just below the sticky header when the card overflows the viewport; on answer reveal the `.quiz-answer` area is **smooth-scrolled** into view so the motion reads as the answer cue. The hook re-fires on card-height changes, so the frame is scrolled in the moment it finishes loading and the answer once it renders
