...
- [ ] When the active show tab closes, the server clears the active slot but does **not** auto-promote a background show client — a frontend left in the "nicht aktiv" state never silently becomes the main show. The empty slot is reclaimed by whichever show **registers** next (a reloading main) or by an explicit `show-claim` ("übernehmen")
...
- **Never interrupt a running show; identity-based reload (server-side)**: each show tab sends a stable per-tab `id` (sessionStorage) with `show-register` / `show-claim`. The server tracks `activeShowId` (the owning frontend's id) and decides activation with the pure `decideShowRegister(slotOccupied, ownerId, registeringId)`:
  ...
  - **Empty slot owned by a DIFFERENT frontend** → ignore (a background/new frontend must NOT silently become main when the owner left — requires explicit claim).
