...
- [x] A client cannot publish a team-state snapshot older than one already in circulation: the server relays a `gamemaster-team-state-v2` write only when its `TeamState.rev` beats the cached rev, and returns the cached value to a rejected writer so it converges
...
- [ ] A newly-opened `/show` tab emits **nothing** to the gamemaster (no `gamemaster-answer`, `gamemaster-controls`, `gamemaster-team-state-v2`, or `gamemaster-question-tally`) until the server confirms it is the active show. Opening a second frontend while a game runs must NOT move the gamemaster off the active show's state — even for the brief window before the new tab receives `{ isActive: false }`. Achieved by starting a prod show tab **gated** (`inactive=true`) at module load, before any emit effect runs; only `{ isActive: true }` un-gates it (dev + non-show tabs start ungated)
- [ ] When a `/show` tab becomes active (via user claim, or by reclaiming its own slot by id match on reconnect/reload), it immediately re-emits its own `gamemaster-answer`, `gamemaster-controls`, `gamemaster-team-state-v2`, and `gamemaster-question-tally` state, so the gamemaster view snaps to the new active's truth
...
- [ ] State channels (`gamemaster-answer`, `gamemaster-controls`, `gamemaster-team-state-v2`, `gamemaster-question-tally`) are cached server-side; on server restart, the active show re-seeds the cache on WS `onopen`
...
- New WS channels (in `server/ws.ts` and `src/services/useBackendSocket.ts` `WsChannel` union):
  ...
  - `gamemaster-team-state-v2` — any client → any client, cached
  - `gamemaster-question-tally` — any client → any client, cached
...
- **Version-guarded tally writes (`gamemaster-question-tally`)**: the per-question tally carries the same Lamport counter as `TeamState`, persisted in `localStorage.questionTallyRev`. Local mutations author `rev + 1`; the three show re-seed paths (`onWsOpen` / `onBecameActive` / `onReemitRequest`) re-assert at the CURRENT rev because they are republishing, not authoring; an inbound payload behind the local rev is refused and the local value re-asserted. Without this the channel was last-write-wins, so a show-side reconnect republished a copy that predated the gamemaster's latest taps and silently reverted them.
  The rev rides INSIDE the map under a reserved `__rev` key rather than wrapping the payload in `{ map, rev }`. `normalizeCorrectAnswersMap` skips any entry whose value is not an object, so an older installed PWA ignores the key instead of misreading it as a game's question map — reshaping this payload is exactly the failure the channel was renamed to avoid.
- **Dropped-write queues**: `sendWs` is a no-op while the socket is reconnecting (1–10s backoff on venue WiFi) and returns `false`. Both state channels check that return value and park the unsent payload in a ref (`pendingTeamsRef` / `pendingCorrectRef`), flushed from `onWsOpen`. A gamemaster tap made inside a reconnect window was otherwise discarded with nothing queued, and the server's cached value then overwrote it.
...
- **Cold-start authority (show tabs only)**: `captureColdStartFlags()` records at module load whether this tab booted with NO team state in localStorage (`team1`, `team2`, `team1Points`, `team2Points`, `scoreHistory`, `doubleNextGame` all absent — the points keys matter because a show run with rosters disabled has no `team1`/`team2` at all and would otherwise look "cold" on every load). While the flag holds, the FIRST inbound team-state carrying any data is dropped and the tab re-asserts its own empty state instead — otherwise a previous session's cached value, or a stale GM/admin tab re-broadcasting its in-memory copy on mount, would silently repopulate a deliberately-cleared show. One-shot per page load, per channel. The re-assert goes through the reducer so it authors a `rev` ABOVE the snapshot it refused; a bare re-send would lose to the version guard and the refused state would return.
  **Only an ACTIVE show tab may fire the gate.** An `isInactiveShowTab()` tab adopts the inbound snapshot verbatim (`remote: true`, keeping the sender's rev) instead of re-asserting. Without that exception the gate turned the standard hardware-failure recovery into total data loss: the projector dies mid-show, the operator opens `/show` on a spare laptop (no localStorage → gate armed), the server pushes the live state at rev 42, the gate refuses it and authors empty at rev 43 — and the moment the operator clicks "übernehmen", `onBecameActive` broadcasts empty@rev43, which outranks 42 and resets points, names, jokers and the whole score history on every device. The same rule applies to the tally gate.
- **Re-emit on active transition**: when a tab transitions inactive → active (claim, or reclaiming its own slot by id match on reconnect/reload), registered `onBecameActive` callbacks re-emit `gamemaster-answer`, `gamemaster-controls`, `gamemaster-team-state-v2`, `gamemaster-question-tally` so the server cache (and every connected GM) snaps from the old active's stale values to the new active's truth.
...
## Channel version
The team-state channel is `gamemaster-team-state-v2`. It was renamed from `gamemaster-team-state`
when `TeamState` grew teams 3 and 4: a stale cached PWA would have normalized a 4-team payload down
to two teams and re-broadcast it, silently destroying the other teams' rosters and points mid-show.
Under a new name an old peer simply fails to sync, which is visible. See [team-count.md](team-count.md).
