...
- [ ] SVG ↔ raster swaps return `409 vector_raster_mismatch` with `oldExt` + `newExt`. The client renders a `.replace-warning` banner (text varies by direction: raster → SVG highlights scalability, SVG → raster warns about losing scalability) and replaces the primary confirm with a `be-btn-warning` button labelled **"Trotzdem ersetzen — <OLD> → <NEW>"**. Submitting that button retries with `force: true`, which also bypasses the smaller-image guard.
...
- [ ] Clicking a candidate **selects** it (highlight ring via `selectedUrl`) and renders a large single-pane preview block (`.replace-compare.replace-compare--single` → `.replace-compare-pane`) below the result grid showing the full-resolution image, dimensions, source, and title. The actual download is gated behind an explicit **✓ Herunterladen** confirm button in the modal footer — no immediate download on click.
- [ ] Clicking the image in the preview pane opens it in the shared `.image-lightbox` portal (Esc closes).
- [ ] Clicking the already-selected candidate again deselects it (preview pane disappears). Submitting a new search (form submit on `<ImageSearchPanel>`) clears the current candidate so a stale pick from the previous result set doesn't leak through.
- [ ] Confirming calls the existing `POST /api/backend/assets/images/download-url` endpoint via `downloadImageFromUrl('images', url, subfolder, desiredName)`. While the request is in flight, the selected candidate carries the `is-busy` "Lade…" badge, other candidates are disabled, and the close (✕) button + subfolder dropdown + provider toggle are all disabled.
...
- [ ] On error: the modal stays open, an inline `.replace-error` banner shows the message, the candidate stays selected, and the confirm button becomes interactive again.
...
- [ ] Clicking a candidate **selects** it and renders a large single-pane preview block (`.replace-compare.replace-compare--single` → `.replace-compare-pane`) below the result grid showing the full-resolution image, dimensions, source, and title; the image is clickable to open the shared `.image-lightbox`. The actual download is gated behind a **✓ Herunterladen** button rendered in a `.picker-footer` below the preview — no immediate download on click. Clicking the same candidate again deselects it; submitting a new search clears the candidate. Confirming calls `downloadImageFromUrl('images', url, subfolder, desiredName)` and on success closes the picker via the existing `onSelect` contract (i.e. the path is written to the question via the `AssetField.onChange`). While the request is in flight the candidate carries the `is-busy` "Lade…" badge, other candidates are disabled, and the mode toggle + subfolder dropdown are disabled. On error the modal stays open with a `.replace-error` banner and the candidate stays selected.
