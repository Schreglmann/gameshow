...
- The visible countdown is rendered by `src/components/common/DeadlineTimer.tsx` (absolute-deadline driven, ring + tick-from-30s + synth finish). The per-question `src/components/common/Timer.tsx` (SimpleQuiz / BetQuiz / WerKenntMehr) also ticks (low from 30s, high in the final 10s) and shares the same synthesized finish sound. Both pull `playTimerTick` / `playTimerEnd` from the shared `src/utils/timerSound.ts` (one Web Audio context, no binary asset).
