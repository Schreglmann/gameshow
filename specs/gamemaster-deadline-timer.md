...
- [ ] On expiry, the ding sound plays (bundled from `public/sfx/timer-end.mp3` so it ships with every PWA and isn't dependent on `local-assets`) and the display shows "Zeit abgelaufen!". The URL is resolved relative to the PWA base via `import.meta.env.BASE_URL` (`/show/sfx/timer-end.mp3` in the deployed show build, `/sfx/timer-end.mp3` in dev) — a root-absolute `/sfx/...` 404s in production because the show bundle is served under `/show/`
