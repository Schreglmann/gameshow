...
- [x] The audio field's `MiniAudioPlayer` honours the trim of the field it belongs to, **whether or not the timeline is expanded**: pressing play starts at `start` (not at 0), playback stops at `end`, and with loop enabled it restarts at `start`
- [x] With a trim set, that `MiniAudioPlayer` represents the **clip**, not the file: its bar spans `start → end`, its time display counts from `start` (e.g. `0:03 / 0:12`), and clicking the bar seeks inside the clip
...
### Audio field mini player (admin)
- `MiniAudioPlayer` takes optional `start` / `end` / `loop` props; `AssetField` forwards them as `audioStart` / `audioEnd` / `audioLoop`, so every question form that offers a trim passes the same values it passes to its `AudioTrimTimeline`
- Boundary handling matches the timeline: play from outside the region jumps to `start`, reaching `end` pauses (or restarts at `start` when looping). Both components enforce it on the same shared element; the checks are idempotent, so an expanded timeline and its mini player never fight
- Without `start`/`end` the mini player behaves exactly as before (full file, absolute times)
