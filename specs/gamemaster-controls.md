...
- [x] A **Vollbild** toggle appears in the toolbar (between the toggle cluster and the countdown) while the show is displaying an image/video, and opens/closes a fullscreen overlay on the show. It is a toolbar-local control gated on the `fullscreenAvailable` flag, sends `toggle-fullscreen` (handled centrally in `BaseGameWrapper`), and reflects the live `fullscreenOpen` state — see [gamemaster-fullscreen.md](gamemaster-fullscreen.md)
...
- The gamemaster screen renders a **toolbar** (`<div class="gm-toolbar">`) holding three logical groups, each a single flex child so they stay visually distinct: the **toggle cluster** (`<div class="gm-toggle-group">`, the four host-only toggle buttons below), the **countdown group** (`.gm-deadline-group`), and the **scroll group** (`.gm-scroll-group`). The toolbar's inter-group `gap` is deliberately much wider than each group's tight internal gap, so the clusters read as separate even though they sit in one row. The toggle cluster (floated in the left gutter at ≥1280px, inline above the card below that — see the responsive note at the end of this section) contains:
  ...
  - **Hide answers** (`.gm-answers-toggle` / `--hidden`) — when on (label "Antworten zeigen"), every answer-bearing element of the GM card is suppressed: the current answer (replaced by a dimmed "Antworten versteckt" hint), the answer image, the extra-info block and the whole next-question preview; a `ranking` answer list stays clickable with masked texts. Lets the host show the GM screen to the players without revealing anything. **Default off** (label "Antworten verstecken"), normal highlight convention. Persisted in `localStorage` under key `gm-hide-answers`. See [gamemaster-hide-answers.md](gamemaster-hide-answers.md).
  - **Pause screen** (`.gm-hold-toggle` / `--active`) — drives the cached `show-hold` channel instead of `localStorage`; see [gamemaster-cockpit.md](gamemaster-cockpit.md).
- The lock / image / hide-answers toggles are **per-device only** (no WebSocket sync) — two gamemasters on different devices can independently choose lock / image / answer-visibility state.
- The visibility toggles only affect the gamemaster view; the player-facing `/show` projector is unaffected.
- Between the toggle cluster and the deadline row, the toolbar renders a **Vollbild toggle** (`.gm-fullscreen-toggle`) — a GM-toolbar-local control (NOT part of `GamemasterControl[]`) shown only while `phase === 'game'` and the show reports `fullscreenAvailable`. It sends `toggle-fullscreen` (handled in `BaseGameWrapper`), and its label / highlight reflect `fullscreenOpen`. See [gamemaster-fullscreen.md](gamemaster-fullscreen.md).
...
- The same payload carries optional `fullscreenAvailable` / `fullscreenOpen` booleans so the GM toolbar can render the Vollbild toggle only while media is on screen and reflect its open state. See [gamemaster-fullscreen.md](gamemaster-fullscreen.md).
