...
- **Atmosphere stack (immersive, body + ::before / ::after — no DOM changes; renders on `html` in-app and on `.theme-preview-panel` in the showcase):**
  ...
  - **`::after`** — one cohesive, **hand-drawn Hogwarts-night silhouette** as a **single inline-SVG** layer spanning the full width (sized `100% auto`, anchored `center bottom`), layered over a soft warm horizon glow. The SVG has a **wide aspect (viewBox 1600×330)** so, bottom-anchored, it renders as a short band low in the viewport — the castle sits in roughly the bottom 30 % with night sky above (a taller viewBox would push it up; trimming the cliff foreground moved it down). The SVG is **one continuous landscape** — a clean solid **cliff plateau** + the castle + Forbidden-Forest pines framing the **extreme edges** — so the sides stay populated beside the centered content card and the bottom edge is filled. The "Hogwarts" read comes from a **dense pile of slender pointed (witch-hat) towers at many heights** sprawling across the cliff (NOT a row of crenellated fortress keeps), with **one dominant Astronomy Tower** — tall and slim — towering over everything left-of-centre, and a second tall tower (clock face) right-of-centre. The **Great Hall** is the lit centrepiece: a steep ridge-spired roof, a **round rose window**, and a row of tall arched lit windows. Receding **back-spires** in a second, darker stone tone give depth; warm gold lit windows read across the whole castle. The cliff is a **wide flat-topped plateau** (top ≈ y250, every building footing ≥ 10 px below it so **nothing floats**) with **craggy drop-off edges** into the forest — no rock facets/shading, which earlier read as confusing ledge/plane artifacts. Generated by [`scripts/generate-hogwarts-scene.cjs`](../scripts/generate-hogwarts-scene.cjs) (edit + re-run `--write-css` to regenerate the baked data-URI); kept in-repo like the Ländergrenzen map generator. The `max-width: 576px` query switches to `auto 26vh` so phones keep the iconic centre (Great Hall + flanking towers) at a usable size rather than a thin sliver. **History:** earlier drafts used an embedded reference **PNG** pasted over a *separate* forest SVG — two disconnected layers that "made no sense"; then a flat row of buildings; then a draft that floated buildings (a humped cliff dipping below the footing line) and felt too centred; then a wide complex that read a bit "generic fortress" (crenellated keeps). The current scene is the resolution: one silhouette, flat plateau (nothing floats), wide spread, leaning into Hogwarts' dense pointed-tower skyline + dominant Astronomy Tower + rose-windowed Great Hall, sitting low in the frame.
...
- **Responsive simplification:** `@media (max-width: 576px)` switches the castle scene from a full-width band (`100% auto`) to a height-based `auto 26vh` so phones keep the iconic centre (Great Hall + flanking towers) at a usable size; the star/candle ceiling stays as-is.
- **Reduced motion:** the whole Harry Potter atmosphere is **fully static** (no candle flicker, no twinkle — pulsing the full-viewport layer read as the page flashing), so reduced-motion users see the identical scene; nothing needs gating.
