...
- [ ] Fixing a question's text/answer in a `games/*.json` shows live; if it's the on-screen question, it updates in place without losing position. **This holds for `randomizeQuestions` games too** — the shuffle order stays fixed across the live re-fetch, so the edit lands on the same question rather than re-dealing the deck.
...
- [ ] **Adding, removing or disabling** questions of the currently-playing game does not re-deal the deck either: additions queue at the end, a removed already-asked question pulls the current one back by one so the same question stays on screen, and a removed pending question changes nothing. Owned by [live-question-order.md](live-question-order.md), which has the full criteria.
...
- [ ] An edit made while the show's WebSocket was **disconnected** (server restart, laptop sleep, network blip) is recovered automatically: on reconnect the running game re-fetches and reflects the missed change — without a manual reload. Because `content-changed` is ephemeral (no replay), a missed event would otherwise persist until the next edit or a reload; the reconnect re-fetch self-heals it.
...
- **Ranking reveal** ([src/components/games/Ranking.tsx](../src/components/games/Ranking.tsx)): the progressive reveal is a positional prefix, so a live edit to the current question's answers reconciles `revealedCount` (`diffSingleElement` — now shared from [src/utils/questionOrder.ts](../src/utils/questionOrder.ts) — plus a baseline ref, gated to the same **question**, keyed by its slot rather than its index so a shifted index still reconciles instead of reading as navigation): deleting a *revealed* answer decrements the count (item gone, no hidden answer slides in), deleting an *unrevealed* answer leaves it unchanged, and a same-length text edit only clamps (the corrected text stays revealed).
- **Current game** ([src/components/screens/GameScreen.tsx](../src/components/screens/GameScreen.tsx)): subscribe to `content-changed`; on `config || games`, re-fetch `/api/game/:index` **without blanking** (`gameData` swaps in place). A request-id ref guards against navigation/refresh fetches resolving out of order. **Also re-fetch on every WS (re)connect** (`useWsOpen`): `content-changed` is ephemeral (no replay), so an edit made while this client's socket was down would be silently lost — the reconnect re-fetch recovers it. The connect-time refetch is an idempotent duplicate of the mount fetch (same data, no blank, request-id guarded), so it is safe even when GameScreen is the socket's first subscriber. The render keys `<GameFactory>` on `gameId` (the gameOrder ref), which determines whether the swap preserves or resets:
  - **Same `gameId`** (an edit to the playing game): no remount — the component's local phase survives, the question list recomputes (picking up edits/additions/removals), and the host stays on the same question. Every game takes its list from `useQuestionOrder` ([src/hooks/useQuestionOrder.ts](../src/hooks/useQuestionOrder.ts)) and its current index from `useLiveQuestionIndex`, which together reconcile the new source array against the order already being played instead of re-deriving it. That covers a *length* change too, which the earlier seed-only approach could not: see [live-question-order.md](live-question-order.md) for the diff, the slot-identity model and the full behaviour table.
...
- Deleting *every* question renders the card blank until the gamemaster navigates (components already null-guard — no crash).
- The randomized-order stability guarantee now also covers **length** changes — adding, removing or toggling `disabled` on a question no longer re-orders the rest. Owned by [live-question-order.md](live-question-order.md), including the remaining edge cases (a same-length delete+add is read as an edit in place; deleting the "Beispiel" question promotes its successor).
