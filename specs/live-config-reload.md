...
- **Current game** ([src/components/screens/GameScreen.tsx](../src/components/screens/GameScreen.tsx)): subscribe to `content-changed`; on `config || games`, re-fetch `/api/game/:index` **without blanking** (`gameData` swaps in place). A request-id ref guards against navigation/refresh fetches resolving out of order. **Also re-fetch on every WS (re)connect** (`useWsOpen`): `content-changed` is ephemeral (no replay), so an edit made while this client's socket was down would be silently lost — the reconnect re-fetch recovers it. The connect-time refetch is an idempotent duplicate of the mount fetch (same data, no blank, request-id guarded), so it is safe even when GameScreen is the socket's first subscriber. The render keys `<GameFactory>` on `gameId` (the gameOrder ref), which determines whether the swap preserves or resets:
  - **Same `gameId`** (an edit to the playing game): no remount — the component's local `qIdx`/phase survive, the shuffle memo recomputes (picking up edits/additions), and `questions[qIdx]` re-reads fresh. For `randomizeQuestions` games the recompute must **not** re-shuffle: every game component takes its shuffled list from `useShuffledQuestions` ([src/hooks/useShuffledQuestions.ts](../src/hooks/useShuffledQuestions.ts)), which holds a per-mount stable seed (lazy `useState`) and feeds it to `randomizeQuestions(..., seed)`. Because the seeded Fisher–Yates permutation depends only on `(seed, list length)` and not on object identity, a same-length content edit re-reads fresh text at the unchanged shuffled position. The seed survives the same-`gameId` re-fetch (no remount) and only regenerates on a real remount / page reload (where `qIdx` resets anyway).
...
- The randomized-order stability guarantee holds only while the **list length** is unchanged. Adding, removing, or toggling `disabled` on a question changes the length, which changes the seeded permutation — the remaining questions may re-order. This is the same behavior as appending/deleting questions in a non-randomized game shifting positions, and is acceptable for the rare live add/remove case.
