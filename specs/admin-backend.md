...
- **Entries per batch:** files and folders appear as rows with a mediaType icon (image / audio / video / other), the original path, the size, and three per-row actions: `Vorschau` (files only), `Wiederherstellen`, `Endgültig löschen`. Restore granularity is at the entry level — when the original deletion soft-deleted a folder, that folder is one row and is restored as a whole; the trash view does not walk inside soft-deleted folders.
- **Multi-select & toolbar:** reuses the existing selection state (`selectedFiles`); trash items are keyed `${batchId}/${originalPath}` (with a trailing `/` for directory entries). The toolbar shows `Wiederherstellen (N)`, `Endgültig löschen (N)`, and `Alle leeren`. Bulk operations group selected keys by `batchId` and issue one server call per batch; conflicts and counts are aggregated into a single result toast.
...
GET    /api/backend/assets/:category/trash           → { batches: TrashBatch[] } — list every surviving soft-delete batch
POST   /api/backend/assets/:category/trash/restore   → { batchId, items? } → { success, restored, conflicts[] }
POST   /api/backend/assets/:category/trash/purge     → { batchId?, items? } → { success, purged, batches } — permanent delete; empties whole category when both are omitted
...
The Papierkorb view (see above) is served by four sibling endpoints; together they cover listing, restore, permanent delete, and binary preview of items in `<categoryDir>/.trash/`.

- `GET /api/backend/assets/:category/trash` — returns `{ batches: TrashBatch[] }` where each `TrashBatch` is `{ batchId, createdAt, expiresAt, sizeBytes, isCurrent, entries[] }` and each entry is `{ originalPath, isDirectory, sizeBytes, mediaType }`. `createdAt` reads the batch directory's `mtimeMs`; `expiresAt = createdAt + 24h` (matches `TRASH_TTL_MS`); `isCurrent` is `true` for the batch that backs the active undo toast (i.e. `batchId === lastDeletion?.batchId`). `mediaType` is derived from the entry extension (`image | audio | video | other`); folders always return `other`. The endpoint treats a missing `.trash` directory as `{ batches: [] }`, not 500.
- `POST /api/backend/assets/:category/trash/restore` — body `{ batchId: string; items?: string[] }`. Each `items` value is an `originalPath`; omit `items` to restore the whole batch. Each entry is renamed back to its original location with `mkdir -p` of the parent. Conflicts (an existing file/folder at the original path) are skipped and reported in `conflicts: string[]`. Restored entries are removed from `lastDeletion`; when `lastDeletion` ends up empty its pointer is nulled so a stale undo toast cannot 404. Returns `{ success: true, restored, conflicts }`.
- `POST /api/backend/assets/:category/trash/purge` — body `{ batchId?: string; items?: string[] }`. Three modes: (1) `{}` empties every batch for the category; (2) `{ batchId }` permanently deletes the whole batch; (3) `{ batchId, items }` purges those specific `originalPath`s from the batch. For each removed entry the deferred NAS delete is queued via `queueNasDelete(category, originalPath)`. For the audio category, derived covers at `/images/Audio-Covers/{basename}.jpg` and their YouTube-thumbnail siblings are removed and the sidecar `audio-cover-meta` entry is dropped — same code path as `purgeDeletionBatch`. Overlapping `lastDeletion` entries are filtered out; an emptied `lastDeletion` is nulled. Returns `{ success: true, purged, batches }` where `batches` is the number of batches that were fully removed.
...
All four endpoints validate `category` via the same `isSafeCategory` allowlist used elsewhere, and every mutating endpoint fires `broadcastAssetsChanged(category)` once on success so connected DAM clients (and any open Papierkorb view) refresh without a manual reload. The existing `POST /api/backend/assets/undo-delete` is unchanged on the wire — internally it now delegates to the same restore helper as `/trash/restore`, so behaviour stays identical.
