...
- [x] Folder move: all snapshot entries under the old folder prefix are rewritten to the new folder prefix when the NAS move op succeeds. When that op is dropped (NAS offline / EBUSY), the **move-intent ledger** stops the next rescan reading the old→new divergence as a mass delete+create, and drives a cheap NAS rename to finish the move — see "Move-intent override" below.
...
### Move-intent override (2026-07-17 follow-up)

The trash-intent override closed the bulk-*delete* hole. A DAM **folder move** hit the same assumption from the other direction and was the reported bug. A move renames the folder locally (synchronously) and enqueues ONE NAS `move` op; on the happy path that op renames the folder on the NAS and `applyOpToSyncSnapshot` prefix-remaps the snapshot — no divergence. But when that op is dropped without updating the snapshot (NAS offline at enqueue → `queueNasSync` early-returns; or the op throws EBUSY/SMB-rename-glitch → shifted off the queue), local = new folder, NAS = old folder, snapshot = old folder. The next `computeSyncOps` (which has **no rename detection**) reads every old-path file as `delete-nas` and every new-path file as `push`. For a folder of >5 files Layer 3 aborts the whole run; for ≥5% of the category Layer 2 vetoes and records a wall of conflicts — "the whole folder was deleted and a new one created".

The fix is a persistent **move-intent ledger** ([server/nas-sync-moves.ts](../server/nas-sync-moves.ts)), the move-shaped analogue of the trash-intent set:

- [x] Sidecar `local-assets/.nas-sync-moves.json` at the assets-tree root (next to `.sync-state.json`, **outside** the walked category folders). Atomic tmp+rename write, validate-on-read, NFC-normalized keys. Entries `{ relFrom, relTo, detectedAt, lastSeenAt }` at folder granularity, keyed by `relFrom`.
- [x] The DAM move endpoint (`POST /api/backend/assets/:category/move`) calls `recordPendingMove` after the local rename and **before** `queueNasMove`/`queueNasMoveCross` (record-before-enqueue: on a healthy NAS the op can succeed and clear the intent before a post-enqueue record would run). Video-reference moves (`wasReference`) are skipped — they never touch the NAS.
- [x] `applyOpToSyncSnapshot`'s `move` case clears the intent (`clearPendingMove`) once the NAS rename lands and the snapshot is remapped.
- [x] **Safety invariant (the load-bearing guard):** the ledger is only a *claim*. `computeMovedAwayRels(moves, localFileKeys, deleteNasRels)` excludes a `delete-nas` rel from the failsafe **only when the destination file physically exists locally under `relTo`** (per file). That local file is the proof the content is not lost — the exact moral equivalent of "the file physically sits in `.trash/`" for the delete case. Its output joins `trashedRels` in the `intentBackedDeleteNasRels` set passed to `applyDeletionSafety` + `checkBulkDelete`. **No change to those two functions** — they already accept the set.
- [x] Both [`startupSync`](../server/index.ts) and [`periodicRescan`](../server/index.ts) reconcile the ledger against the fresh NAS scan (via `reconcilePendingMovesForSync`): expire moves older than `PENDING_MOVE_TTL_MS` (72h → hands control back to the failsafe), self-heal by dropping moves whose `relFrom` no longer exists on the NAS, and — Phase B — for each move that is STRICTLY eligible (NAS still holds `relFrom`, and NOTHING exists at `relTo` in any form) enqueue a cheap NAS `move` op instead of re-uploading the whole folder, then defer the rest of the run. The strict `relTo`-ENOENT guard (via `nasPathExists`) prevents a `rename`→ENOTEMPTY→re-enqueue loop and defers the "NAS already has both" / "only part moved" states to normal convergence.
- [x] The CLI [`sync`](../sync-assets.ts) path reads the ledger read-only (`readPendingMovesSync`) and applies the same failsafe exclusion via `computeMovedAwayRels`, so a manual `sync` during a pending move doesn't false-trip. The CLI never drives the NAS rename or clears the ledger — that stays the server's job.
- [x] **Torn-scan guard.** `startupSync`/`periodicRescan` check the NAS queue only once, then run a multi-second local+NAS walk. A move op that lands mid-walk mutates NAS state under the walk **and** clears its own move-intent on success — producing a torn read (local = new path, NAS = old path) with no ledger entry left to exclude it, which `computeSyncOps` reads as a mass delete+create. A `nasQueueGeneration` counter (bumped per completed op) is snapshotted after the one-time queue guard and re-checked before the failsafe runs; if a queue op landed during the scan, the reconciler **skips the cycle** (logs an informational line, records nothing) and the next scheduled run sees settled state. This closes the class for all op types, not just moves. Without it the run still *aborted safely* (deleted nothing) and self-healed on the next rescan — but it logged an alarming bulk-cap line and transiently recorded conflicts.

**Residual risk (accepted, documented).** Excluding moved rels from Layer 2's local-loss numerator could, if ≤5 **other** files in the same category are *genuinely* lost locally during the exact window of a live move, drop the folder ratio below 5% and let those losses propagate. But those genuine losses are not intent-backed, so Layer 3 still counts them and aborts if >5; every propagated delete is a `softDelete` (Layer 1, 30-day recoverable trash); and this is the identical residual the trash-intent exclusion already accepts. The *moved files themselves* can never be masked (the per-file local-counterpart guard). Short TTL keeps the window small. **Precondition:** this stays safe only while `delete-nas` routes through `softDelete` — never `rm`.
