...
describe('useQuestionOrder', () => {
  it('keeps the shuffled order stable across re-renders with a new questions reference', () => {
    ...
    const { result, rerender } = renderHook(
      ({ questions }) => useQuestionOrder(questions, true).questions,
  ...
  it('does not shuffle when randomization is disabled', () => {
    ...
    const { result } = renderHook(() => useQuestionOrder(questions, false).questions);
  ...
  it('actually randomizes the order when enabled (probabilistic)', () => {
    ...
    const { result } = renderHook(() => useQuestionOrder(questions, true).questions);
  ...
  describe('stable order across remounts with a gameId', () => {
    ...
    it('reproduces the SAME randomized order when re-mounted with the same gameId', () => {
      ...
      const questions = makeQuestions(20);
      const first = renderHook(() => useQuestionOrder(questions, true, undefined, 'georgs-quiz/v1').questions);
      const firstOrder = first.result.current.map(q => q.id);
      first.unmount();

      const second = renderHook(() => useQuestionOrder(questions, true, undefined, 'georgs-quiz/v1').questions);
    ...
    it('without a gameId, a fresh mount re-shuffles (probabilistic)', () => {
      ...
      const first = renderHook(() => useQuestionOrder(questions, true).questions);
      const firstOrder = first.result.current.map(q => q.id);
      first.unmount();
      const second = renderHook(() => useQuestionOrder(questions, true).questions);
