...
const isEmpty = (q: ColorGuessQuestion) => !q.answer.trim() && !q.image;
...
function ColorPiePreview({ image, size = 40, onClick }: { image: string; size?: number; onClick?: () => void }) {
  ...
  return (
    <div style={style} onClick={onClick} title={onClick ? 'Farbverteilung anzeigen' : undefined}>
      {colors === null ? (
        <span style={{ fontSize: 10, color: 'rgba(var(--text-rgb), max(0.4, var(--text-fade-floor, 0)))' }}>…</span>
      ) : colors.length === 0 ? (
        <span style={{ fontSize: 10, color: 'rgba(var(--text-rgb), max(0.4, var(--text-fade-floor, 0)))' }}>–</span>
      ) : (
        ...
        >
          ...
          >
            <div style={{ width: 'min(360px, 50vh)', aspectRatio: '1 / 1' }}>
              {colors === null ? (
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(var(--text-rgb), max(0.5, var(--text-fade-floor, 0)))' }}>Laden …</div>
              ) : (
                <ColorPie colors={colors} highlightIdx={highlightIdx} onHighlight={setHighlightIdx} className="color-pie--fit" />
              )}
            </div>
            <div style={{ minHeight: '1.4em', fontFamily: 'var(--admin-font-mono, monospace)', color: 'rgba(var(--text-rgb), max(0.8, var(--text-fade-floor, 0)))' }}>
