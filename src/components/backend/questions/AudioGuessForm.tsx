      ...
      {displayQuestions.map((q, i) => {
        ...
        >
          <div className="question-block-row">
            ...
            {/* Compact badges */}
            {q.answerImage && (
              <img src={coverUrl(q.answerImage) ?? q.answerImage} alt="" style={{ height: 40, width: 40, objectFit: 'contain', borderRadius: 4, border: '1px solid rgba(var(--glass-rgb), 0.12)', background: 'rgba(0,0,0,0.3)', opacity: 0.6, flexShrink: 0 }} title={`Bild: ${q.answerImage}`} />
            )}
            {q.audio && hasTrim(q) && (
