...
const isEmpty = (q: QuizjagdFlatQuestion) => !q.question.trim() && !q.answer.trim();
    ...
    <div>
      ...
      {displayQuestions.map((q, i) => {
        ...
        <div
          ...
          style={{ borderLeftWidth: 3, borderLeftStyle: 'solid', borderLeftColor: DIFF_STYLES[q.difficulty]?.borderColor ?? 'transparent', display: !isVirtual && diffFilter !== null && q.difficulty !== diffFilter ? 'none' : undefined }}
        ...
        >
          <div className="question-block-row">
            ...
            <span className="question-num">{isVirtual ? 'Neu' : `#${i + 1}`}</span>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {([3, 5, 7] as const).map(d => (
                <button
                  key={d}
                  className="be-icon-btn quizjagd-diff-btn"
                  style={{ padding: '2px 9px', fontSize: 'var(--admin-sz-10, 10px)', lineHeight: 1.2, ...(q.difficulty === d ? DIFF_STYLES[d] : {}) }}
