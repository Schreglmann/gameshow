...
interface Props {
  ...
  onChange: (questions: Q1Question[]) => void;
  otherInstances?: string[];
  onMoveQuestion?: (questionIndex: number, targetInstance: string) => void;
}

const empty = (): Q1Question => ({ Frage: '', trueStatements: ['', '', ''], wrongStatement: '' });
const isEmpty = (q: Q1Question) =>
  !q.Frage.trim() &&
  q.trueStatements.every(s => !s.trim()) &&
  !q.wrongStatement.trim() &&
  !q.answer?.trim();
...
export default function Q1Form({ questions, onChange, otherInstances, onMoveQuestion }: Props) {
  ...
  const update = (i: number, patch: Partial<Q1Question>) => {
    let next: Q1Question[];
