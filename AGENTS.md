...
npm run verify:full  # every gate, unscoped — before a live show
...
| `server/nas-sync-moves.ts` | Move-intent ledger sidecar (`local-assets/.nas-sync-moves.json`) recording DAM folder/file moves whose NAS-side rename isn't yet confirmed. Lets the sync recognize an old→new folder divergence as an in-flight *move* (not data loss) and drive a cheap NAS rename instead of re-uploading — the move-shaped analogue of the trash-intent set. Pure `computeMovedAwayRels` excludes a `delete-nas` from the failsafe **only** when the destination file exists locally (safety invariant). Consumed by `startupSync`/`periodicRescan` + the CLI `sync` — see [specs/sync-bidirectional.md](specs/sync-bidirectional.md) "Move-intent override" |
| `server/nas-sync-prefs.ts` | Operator-configurable NAS sync settings sidecar (`nas-sync-prefs.json`): the NAS `basePath` (resolved into `NAS_BASE` at boot — path change needs a restart) + an `enabled` on/off toggle (applies live). Editable in the admin System-tab "NAS-Synchronisation" card via `/api/backend/nas-sync-config`; default keeps the original hardcoded path — see [specs/nas-sync-config.md](specs/nas-sync-config.md) |
...
| `src/utils/cityCompass.ts` + `src/data/cities.generated.ts` + `src/components/common/CompassRose.tsx` | The `city-compass` game type's pure geometry (great-circle distance + bearing, label layout) and its "Auto" neighbor selection, the curated ~3100-city dataset those run against, and the inline-SVG rose all three of show, admin preview and ThemeShowcase render. The dataset is generated by `scripts/generate-city-dataset.ts` from the `all-the-cities` devDependency (GeoNames, CC BY 4.0) — regenerate with `npm run cities:generate`; only the admin imports it, and only dynamically. Coordinates live in the game JSON, so the show renders offline and an admin city swap reaches it live — see [specs/games/city-compass.md](specs/games/city-compass.md) |
...
| `src/services/saveQueue.ts` | Module-scope save queue behind every admin autosave (config + games). Owns the 800 ms debounce, per-file write serialization, and retry-with-backoff — all **outside React**, because the admin panes unmount on a tab switch and an effect-owned timer was silently discarding the edit. Also holds the cross-tab self-write set, since a save can land after its pane is gone. Consumed via `enqueueSave`/`flushSave`/`discardSave` + the shell's `SaveStatusIndicator` — see [specs/admin-save-queue.md](specs/admin-save-queue.md) |
...
| `src/utils/rulesPreset.ts` | Shared preset resolver + `PLACEHOLDER_TASK_LINE`, used by both server (`loadGameConfig`) and admin client. A preset stores up to three **team-count bands** — `rulesSolo` (0–1 teams), `rules` (2), `rulesMulti` (3–4) — because "beide Teams" is wrong at 3–4 and meaningless at 0–1, where whole lines are dropped rather than reworded. `rulesTeamBand()` / `presetRulesForTeamCount()` are the only place the band is decided |
...
| `src/utils/showTitle.ts` | What the show calls itself — the landing-page `<h1>`, the gamemaster's start/summary label and the browser tab title. Operator-configurable on two levels (global `AppConfig.showTitle`, per-gameshow `GameshowConfig.showTitle`) and resolved in exactly ONE place: `resolveShowTitle()`, called by `GET /api/settings`, so no client re-derives the precedence. Also owns the `show:title` localStorage cache the pre-mount `emitCachedGamemasterState()` reads — see [specs/show-title.md](specs/show-title.md) |
| `src/utils/pointMode.ts` | How a gameshow turns a game result into points: `PointMode` (`positional` / `flat` / `per-correct-answer`, per gameshow via `GameshowConfig.pointMode`), the tolerant `normalizePointMode()` and `gamePointValue()`. Resolved in exactly ONE place — `BaseGameWrapper` — so no game component passes or overrides a point value. `per-correct-answer` pays out the gamemaster's tally (`tallyTotals`) and renders the award screen read-only; the four types with `usesCorrectAnswerTally: false` keep their own inline scoring — see [specs/point-system.md](specs/point-system.md) |
...
| `src/utils/teamColors.ts` + `src/hooks/useTeamColorVars.ts` + `src/components/common/TeamDot.tsx` | Operator-picked colour per team, marking every team surface in all three zones with an accent edge plus a dot. The master switch is applied in exactly ONE place — `resolveTeamColors()`, called by `GET /api/settings` — so an empty map already means "mark nothing" and `teamColorsEnabled` never reaches a client. From there it travels through **CSS, not props**: `useTeamColorVars` publishes `--team1-color` … `--team4-color` on `<html>` (one effect in `GameProvider`, which all three PWAs mount) and each surface's whole contribution is a `data-team` attribute, which resolves `--team-color: var(--teamN-color, var(--teamN-house, transparent))` in `src/styles/team-accent.css`. **An accent token there holds the GEOMETRY only and each use site appends `var(--team-color)`** — a `var()` nested inside a custom property is substituted on the element that DECLARES it (`<html>`, where `--team-color` does not exist), which makes every `box-shadow` reading it compute to `none`, silently and everywhere. Three per-team states: absent = default palette, `''` = the theme's house colour, `#rrggbb` = override — see [specs/team-colors.md](specs/team-colors.md) |
| `src/components/common/RunOfShowPanel.tsx` + `GmConfirmDialog.tsx` + `src/hooks/useShowNavigationCommands.ts` | The gamemaster "Ablauf" panel: the whole running order (Startseite · Regelwerk · games · Zusammenfassung) with the show's live position, and a confirmed jump to any entry. Reads `GET /api/run-of-show` (the GM zone otherwise only ever mirrors the CURRENT game) and emits `goto:home` / `goto:rules` / `goto:game-<index>` / `goto:summary` on the existing `gamemaster-command` channel. The show side is ONE app-level listener in `AppContent` — no mounted screen owns both a router and a command listener. **A GM overlay must set `data-gm-no-nav` and raise `onOverlayActiveChange`**, or `GamemasterScreen`'s document-level click/key handlers advance the show behind it. It must also **portal to `document.body`** — the panel is a fixed/transformed/overflow-scrolling ancestor, which on iOS Safari becomes the containing block for `position: fixed`, clipping the overlay to the sidebar — see [specs/gamemaster-run-of-show.md](specs/gamemaster-run-of-show.md) |
...
| Endpoint | Returns |
|----------|---------|
| `GET /api/settings` | `SettingsResponse` (incl. the resolved `showTitle`, `teamCount`, `pointMode`, `teamColors` + `incompatibleGames`) |
| `GET /api/game/:index` | `GameDataResponse` |
| `GET /api/run-of-show` | `RunOfShowResponse` — the active gameshow's running order with titles (backs the gamemaster "Ablauf" panel) |
...
**State anti-patterns:**
- **Never** write directly to `localStorage` from a component — gameplay state goes through the reducer. **Documented exception:** device-local UI flags (GM lock / answer visibility in [GamemasterScreen.tsx](src/components/screens/GamemasterScreen.tsx) + [useGamemasterSync.ts](src/hooks/useGamemasterSync.ts), the run-of-show expand flag in [RunOfShowPanel.tsx](src/components/common/RunOfShowPanel.tsx), theme selection in `ThemeContext`) persist directly — they are per-device UI state, not show state
...
- Point value follows the gameshow's `pointMode` and is derived once, in `BaseGameWrapper`, from `currentIndex` — never pass or hardcode one (see [specs/point-system.md](specs/point-system.md))
...
- **Don't** hardcode point values or reintroduce a `pointValue` prop — `BaseGameWrapper` derives it from `currentIndex` + the gameshow's `pointMode`
