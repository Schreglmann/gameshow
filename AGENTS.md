...
| `src/utils/cityCompass.ts` + `src/data/cities.generated.ts` + `src/components/common/CompassRose.tsx` | The `city-compass` game type's pure geometry (great-circle distance + bearing, label layout) and its "Auto" neighbor selection, the curated ~2800-city dataset those run against, and the inline-SVG rose all three of show, admin preview and ThemeShowcase render. The dataset is generated by `scripts/generate-city-dataset.ts` from the `all-the-cities` devDependency (GeoNames, CC BY 4.0) — regenerate with `npm run cities:generate`; only the admin imports it, and only dynamically. Coordinates live in the game JSON, so the show renders offline and an admin city swap reaches it live — see [specs/games/city-compass.md](specs/games/city-compass.md) |
