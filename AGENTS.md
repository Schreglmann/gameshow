...
| `server/spellcheck.ts` + `server/spellcheck-allowlist.ts` | German/English spell + grammar check (admin tab labelled **Korrektur**, tab id `spellcheck`). `spellcheck.ts` proxies to LanguageTool (`LANGUAGETOOL_URL`, default public API; `LANGUAGETOOL_LANGUAGE`, default `auto`). Two-pass auto-detect (batch-detect, then re-check only flagged fields individually so English answers aren't flagged as German) + a per-field response cache **persisted to a gitignored `.spellcheck-cache.json`** sidecar (survives server / `tsx watch` restarts; disabled under vitest) + concurrent requests governed by a **sliding-window** rate limiter (burst up to ~18 req / ~70 KB per minute on the public API, only throttling once a window fills — never a fixed per-request delay). Live throttle state is exposed via `GET /api/backend/spellcheck/rate-status` and surfaced as a banner in the scan UI. `spellcheck-allowlist.ts` is the repo-root `spellcheck-allowlist.json` sidecar holding the global on/off flag (default off), allowed words, and ignored-match fingerprints. Routes under `/api/backend/spellcheck/*`. Client: `SpellcheckSettingsContext` (global gate), `LektoratTab` (whole-show scan + dictionary), `SpellCheckPanel` (report), `SpellField`/`SpellCheckContext` (inline underlines), `src/utils/spellcheckFields.ts` (prose extractor) + `spellcheckFingerprint.ts` (shared fingerprint) — see [specs/spellcheck.md](specs/spellcheck.md). |
