...
| `server/nas-reachability.ts` | Freeze-proof NAS access. `isNasMounted()`/`isNasReachable()` is a NON-blocking cached flag refreshed by a single-flight, timeout-bounded background probe; `nasStat`/`nasPathExists` are bounded async wrappers. A stale `/Volumes/Georg/Gameshow/Assets` network mount (network dropped, mount not yet torn down) makes any sync `stat`/`read` block UNINTERRUPTIBLY in the kernel — the old blocking `statSync(NAS_BASE)` froze the whole event loop and made the process unkillable. NEVER do synchronous NAS I/O on the main thread; route every NAS touch through this module (or async `fs/promises`) — see [specs/nas-freeze-resilience.md](specs/nas-freeze-resilience.md) |
...
| `ranking` | JSON `questions[]` (question + ordered `answers[]`; progressive reveal; optional `answerAudio` + `answerAudioTrigger` `first`/`all`; optional per-question `items[]` bare candidates shown shuffled during guessing so teams sort given items — `items` are distinct from the solution-bearing `answers`, and their presence alone enables the pool) | `AwardPoints` (host picks winner) |
...
| `random-frame` | JSON `questions[]` (video + answer; server extracts a random still frame at runtime via `GET /api/random-frame`, auto-skipping black frames) | `AwardPoints` (host picks winner). GM can re-roll the current and next frame ("Neues Bild" / "Nächstes Bild"). Admin "Bilder herunterladen" prerenders 3 fallback frames per question so the show works when the source video (often NAS-only) is unreachable; live extraction is always preferred when the source is reachable. See [specs/games/random-frame.md](specs/games/random-frame.md) |
...
| Frontend verification | After any frontend change (`.tsx`, `.css`, UI text), use Playwright MCP to take screenshots at **375px** (phone), **768px** (tablet), **1024px** (laptop), and **1920px** (projector) to verify the change is responsive and visually correct at all sizes. Screenshots taken for decision-making (e.g. choosing between visual options, confirming a design direction) must be saved as PNG files in the project root (e.g. `preview-1.png`) — not just displayed in chat, where they are lost after the conversation ends. The root is gitignored for PNGs so they don't pollute the repo |
...
When screenshots are needed to make a decision (choosing between options, confirming a direction, comparing variants), save them as PNG files in the project root (e.g. `preview-1.png`) so the evidence persists beyond the current chat session. The root is gitignored for PNGs.
