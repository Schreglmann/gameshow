...
| `src/utils/safePlay.ts` + `src/hooks/usePreloadAsset.ts` + `src/hooks/useGmConnected.ts` + `src/components/common/RetryImage.tsx` + `src/components/common/AssetReloadButton.tsx` | Asset-resilience primitives shared by audio/video games. `safePlay()` wraps `HTMLMediaElement.play()` with retry + muted-autoplay fallback; `usePreloadAsset` eagerly prefetches next-question audio + images and releases the decoder on cleanup; `<RetryImage>` is a drop-in `<img>` wrapper that retries with `?v={attempt}` cache-bust only on retry (preserving back-nav HTTP cache hits); `useGmConnected()` subscribes to the `gm-presence` WS channel; `<AssetReloadButton>` is the inline frontend fallback shown only when no GM is connected. Used by SimpleQuiz / AudioGuess / Bandle / VideoGuess after a live-show bug where transient network failures silently broke single questions — see [specs/asset-resilience.md](specs/asset-resilience.md). |
