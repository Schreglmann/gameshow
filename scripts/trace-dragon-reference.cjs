 ...
 *
 * Reference: the public-domain heraldic Welsh Dragon (Y Ddraig Goch) from Wikimedia Commons
 * — a classic rampant European dragon. NOT committed; pass its path as the first arg
 * (defaults to $TMPDIR/welsh-dragon.svg). Fetch it with:
 *   curl -sL "https://commons.wikimedia.org/wiki/Special:FilePath/Welsh_Dragon_(Y_Ddraig_Goch).svg" -o "$TMPDIR/welsh-dragon.svg"
...
const SRC = process.argv[2] || (process.env.TMPDIR || '.') + '/welsh-dragon.svg';
