...
When asked to add a new game type, follow the full workflow defined in [`skills/add-gametype/SKILL.md`](../skills/add-gametype/SKILL.md). Read that file before writing any code.
...
5. **Verify** — run `npm run verify`; fix all failures. It picks the gates your change set
   affects (lint, typecheck, `validate`, `contracts:lint`, tests) and escalates to the full suite by
   itself when shared code is touched — see `AGENTS.md §7`

The detailed instructions for each step (file paths, code patterns, conventions) are in `skills/add-gametype/SKILL.md`.
